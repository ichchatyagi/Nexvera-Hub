...
import { ChevronRight, Star, BookOpen, Layout, Users, Monitor, UserCheck, Lightbulb } from 'lucide-react';
import { categoryData } from '../data/categoryData';
import heroImg from '../assets/courses-hero.png';
import tuitionImg from '../assets/images/tuition.png';
        ...
        <section className="relative pt-12 pb-20 overflow-hidden bg-transparent">
            ...
            <div className="container mx-auto px-6 lg:px-12 flex flex-col lg:flex-row items-center gap-16 lg:gap-24">
                <div className="flex-1 z-10 text-center lg:text-left">
                    ...
                    <h1 className="text-5xl md:text-7xl lg:text-8xl font-black leading-[0.95] mb-8 text-slate-950 uppercase tracking-tighter">
        ...
        <section className="py-20 bg-white/50 backdrop-blur-sm border-y border-slate-100 overflow-hidden">
            <div className="container mx-auto px-6 lg:px-12 mb-12">
                <div className="flex items-center justify-between">
                    <div>
                        <h2 className="text-3xl font-black text-slate-800 mb-2">Student <span className="text-blue-600">Reviews</span></h2>
                        ...
                        <div key={index} className="flex-shrink-0 w-[450px] p-8 whitespace-normal bg-white rounded-[2.5rem] border border-slate-100 hover:border-blue-200 hover:shadow-2xl hover:shadow-blue-500/10 transition-all duration-500 group">
                            <div className="flex items-center gap-4 mb-6">
                                ...
                                <div className="flex flex-col">
                                    <span className="font-black text-slate-800 text-lg leading-tight">{t.name}</span>
                                    <span className="text-xs text-blue-600 font-black uppercase tracking-widest">{t.role}</span>
                                </div>
                ...
                <div className="flex flex-col lg:flex-row items-center gap-16 mb-20 text-center lg:text-left">
                    <div className="flex-1 order-2 lg:order-1">
                        ...
                        <motion.h2 
                            ...
                            className="text-4xl lg:text-6xl font-black text-slate-900 uppercase tracking-tighter mb-8 leading-[0.95]"
                        >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {features.map((f, i) => (
                        ...
                        >
                            ...
                            <div>
                                <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight mb-3 leading-tight">{f.title}</h3>
...
const CourseCard = ({ category, title, instructor, lessons, rating, reviews, color, icon, level = "Beginner" }) => {
    ...
    return (
        <Link to={`/course/${encodeURIComponent(category)}/${slug}`} className="bg-white rounded-[2.5rem] overflow-hidden shadow-sm border border-slate-100 flex flex-col hover:shadow-2xl hover:shadow-blue-500/10 transition-all duration-500 group">
            <div className={`h-48 bg-gradient-to-br ${color} p-8 flex flex-col justify-between relative overflow-hidden`}>
                ...
                <h3 className="text-xl lg:text-2xl font-black text-white leading-[1.1] uppercase tracking-tight relative z-10">
                    {title}
                </h3>
            </div>
            ...
            <div className="p-6 flex-1 flex flex-col">
                <div className="flex items-center gap-3 mb-4">
                    <div className="w-10 h-10 rounded-xl bg-slate-100 overflow-hidden flex items-center justify-center text-blue-600 font-bold">
                        {instructor.charAt(0)}
                    </div>
                    <div>
                ...
                <div className="mt-auto pt-4 border-t border-slate-50 flex items-center justify-between">
                    <div className="flex items-center gap-1">
                        ...
                        <span className="text-slate-900 font-bold text-xs ml-1">{rating}</span>
                        <span className="text-slate-400 text-[10px] font-bold ml-1">/ {reviews}</span>
                    </div>
                    <div className="flex items-center gap-1 text-slate-400 group-hover:text-blue-600 transition-all">
                ...
                <div className="flex flex-wrap items-center justify-between gap-8">
                    {stats.map((stat, index) => (
                        <div key={index} className="flex flex-col items-center text-center group cursor-pointer">
                            <div className={`w-16 h-16 ${stat.color} rounded-2xl flex items-center justify-center text-2xl text-white mb-4 transition-all duration-300 group-hover:-translate-y-2 group-hover:rotate-6`}>
                                {stat.icon}
                            </div>
                            <h4 className="text-xl font-black text-slate-800">{stat.label}</h4>
                            <p className="text-sm font-bold text-slate-400 mt-1 uppercase tracking-wider">{stat.sub}</p>
                        </div>
                    ))}
                </div>
            </div>
...
const Courses = () => {
    ...
    const [activeCategory, setActiveCategory] = useState(location.state?.category || 'IT & Technology');
                    ...
                    {allActiveCourses.length > 10 && (
                        <div className="mt-16 flex justify-center">
                            <button
                                ...
                                className="bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-600 hover:to-blue-700 text-white font-bold px-6 lg:px-12 py-4 rounded-2xl shadow-xl shadow-cyan-100 transition-all active:scale-95 group"
                            >
