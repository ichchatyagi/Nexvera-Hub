...
export class EnrollmentsService {
  ...
  async checkEnrollment(courseId: string, studentId: string) {
    ...
    return {
      ...
      data: {
        ...
        isCompleted:
          enrollment.progress.percentage >= this.COMPLETION_THRESHOLD,
      },
    };
  }
  ...
  async updateProgress(courseId: string, studentId: string, dto: UpdateProgressDto) {
    ...
    if (currentLessonId) {
      if (!Types.ObjectId.isValid(currentLessonId))
    ...
    if (totalLessons > 0) {
      ...
      enrollment.progress.percentage = Math.min(
        100,
    ...
    if (
      currentLessonId &&
      (dto.lastPositionSeconds !== undefined || dto.videoId)
    ) {
      const lessonId = new Types.ObjectId(currentLessonId);
      const historyItemIndex = enrollment.watch_history.findIndex(
        (item) => item.lesson_id.toString() === currentLessonId,
      );
      ...
      if (historyItemIndex === -1) {
        ...
        if (dto.lastPositionSeconds !== undefined)
          item.last_position_seconds = dto.lastPositionSeconds;
