...
export class AnalyticsService {
  ...
  async getOverview() {
    ...
    const total_students = await this.userRepository.count({
      where: { role: UserRole.STUDENT },
    });
    const total_teachers = await this.userRepository.count({
    ...
    const published_courses = await this.courseModel.countDocuments({
      status: 'published',
    });

    // 3. Enrollments
    const total_enrollments = await this.enrollmentModel.countDocuments();
    const active_enrollments = await this.enrollmentModel.countDocuments({
      status: 'active',
    });
    ...
    const revenue_all_time =
      allTimeTx.reduce((sum, tx) => sum + Number(tx.amount || 0), 0) / 100;
    ...
    const revenue_last_30d =
      last30dTx.reduce((sum, tx) => sum + Number(tx.amount || 0), 0) / 100;
    ...
    const revenue_mtd =
      mtdTx.reduce((sum, tx) => sum + Number(tx.amount || 0), 0) / 100;

    // 5. Live Classes
    const live_classes_scheduled_today =
      await this.liveClassModel.countDocuments({
