...
export class TeacherTuitionService {
        ...
        role === UserRole.ADMIN
          ...
          : course.tuition_meta.subjects.filter(
              (s) =>
                ...
                s.assigned_instructor_ids?.includes(teacherId),
            );
  ...
  async getSubjectTeachingView(
    ...
    const isAssigned =
      role === UserRole.ADMIN ||
      subject.lead_instructor_id === teacherId ||
      subject.assigned_instructor_ids?.includes(teacherId);
    if (!isAssigned)
  ...
  private async getCourseAndAssignedSubject(
    subjectId: string,
    teacherId: string,
  ) {
    if (!Types.ObjectId.isValid(subjectId))
    ...
    const subjectIndex = course.tuition_meta.subjects.findIndex(
      (s) => s.subject_id.toString() === subjectId,
    );
  ...
  async addSection(
    ...
    const { course, subjectIndex, subject } =
      await this.getCourseAndAssignedSubject(subjectId, teacherId);
    ...
    course.tuition_meta!.subjects[subjectIndex].syllabus.push(
      newSection as any,
    );
    await course.save();
  ...
  async updateSection(
    ...
    const sectionIndex = subject.syllabus.findIndex(
    ...
    course.tuition_meta!.subjects[subjectIndex].syllabus[sectionIndex] =
      section;
    await course.save();
  ...
  async addLesson(
    ...
    course.tuition_meta!.subjects[subjectIndex].syllabus[
      sectionIndex
  ...
  async updateLesson(
    ...
    course.tuition_meta!.subjects[subjectIndex].syllabus[sectionIndex].lessons[
      lessonIndex
    ] = lesson;
    await course.save();
